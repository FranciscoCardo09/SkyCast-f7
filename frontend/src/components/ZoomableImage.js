...
const ZoomableImage = ({ src, alt, className = "" }) => {
  ...
  const handleTouchStart = useCallback(
    (e) => {
      if (e.touches.length === 1 && scale > 1 && isModalOpen) {
        setIsDragging(true)
        setDragStart({
    ...
    [scale, position, isModalOpen],
  )

  const handleTouchMove = useCallback(
    (e) => {
      if (isDragging && e.touches.length === 1 && scale > 1 && isModalOpen) {
        e.preventDefault()
        setPosition({
    ...
    [isDragging, scale, dragStart, isModalOpen],
  )
  ...
  useEffect(() => {
    if (isModalOpen && modalRef.current) {
      ...
      // Añadir event listener para wheel
